...
import 'widgets/upload_template.dart';
...
class _GeneralSettingsPageState extends State<GeneralSettingsPage> {
  ...
  FVBCollaborator model = FVBCollaborator(email: '', permission: ProjectPermission.editor);
  ...
  final _openAITokenEditor = TextEditingController();
  final _geminiTokenEditor = TextEditingController();
                    ...
                    children: [
                      ...
                      if (kDebugMode) ...[
                        ...
                        SelectionGroupWidget<ThemeType>(
                          ...
                          title: 'Choose Theme',
                          onChange: (value) {
                            undoWork.add(_userSession.settingModel!.generalTheme, () {
                              _userSession.settingModel!.generalTheme = value;
                              operationCubit.updateUserSetting('generalTheme', value.index);
                              theme.add(UpdateThemeEvent(value));
                              setState(() {});
                            }, (p0) {
                              _userSession.settingModel!.generalTheme = p0;
                              operationCubit.updateUserSetting('generalTheme', p0.index);
                      ...
                      BlocBuilder(
                        ...
                        buildWhen: (_, state) => state is UserDetailsFigmaTokenUpdatedState,
                                  ...
                                  children: [
                                    ...
                                    OutlinedButtonWidget(
                                      ...
                                      onTap: () {
                                        showConfirmDialog(
                                            title: 'Alert',
                                            subtitle: 'Do you want to logout from Figma?',
                                            context: context,
                                            positive: 'Yes',
                                            negative: 'Cancel',
                                            onPositiveTap: () {
                                              _userDetailsCubit.disconnectFigmaAccount();
                                            });
                                      },
                                    )
                                  ],
                          ...
                          SizedBox(
                            ...
                            child: Row(
                              children: [
                                Expanded(
                                  child: AppTextField(
                                    controller: _openAITokenEditor..text=_userSession.settingModel?.openAISecretToken??'',
                                    hintText: 'Secret Key',
                                    height: 45,
                                  ),
                                ),
                                ...
                                FilledButtonWidget(
                                  text: 'Done',
                                  onTap: () {
                                    String? token = _openAITokenEditor.text;
                                    if (token.trim().isEmpty) {
                                      token = null;
                                    }
                                    if (_userSession.settingModel?.openAISecretToken != token) {
                                      _userSession.settingModel!.openAISecretToken = token;
                                      operationCubit.updateUserSetting('openAISecretToken', token);
                          ...
                          SizedBox(
                            ...
                            child: Row(
                              children: [
                                Expanded(
                                  child: AppTextField(
                                    controller: _geminiTokenEditor..text=_userSession.settingModel?.geminiSecretToken??'',
                                    hintText: 'Secret Key',
                                    height: 45,
                                  ),
                                ),
                                ...
                                FilledButtonWidget(
                                  text: 'Done',
                                  onTap: () {
                                    String? token = _geminiTokenEditor.text;
                                    if (token.trim().isEmpty) {
                                      token = null;
                                    }
                                    if (_userSession.settingModel?.geminiSecretToken != token) {
                                      _userSession.settingModel!.geminiSecretToken = token;
                                      operationCubit.updateUserSetting('geminiSecretToken', token);
                                      componentGenerator.initialize();
                                      setState(() {
                      ...
                      if (!kIsWeb) ...[
                        15.hDivider,
                        _buildPathSelection('Flutter Path ', _userSession.settingModel?.otherSettings?.flutterPath,
                            (value) {
                          _userSession.settingModel?.otherSettings?.flutterPath = value;
                          operationCubit.updateUserSetting(
                              'otherSettings', _userSession.settingModel?.otherSettings?.toJson());
                        }),
                        15.hDivider,
                        _buildPathSelection('Project Path ', _userSession.settingModel?.otherSettings?.projectPath,
                            (value) {
                          _userSession.settingModel?.otherSettings?.projectPath = value;
                          operationCubit.updateUserSetting(
                              'otherSettings', _userSession.settingModel?.otherSettings?.toJson());
                        })
                      ],
                            ...
                            StatefulBuilder(builder: (context, setState2) {
                              return CustomDropdownButton<Screen>(
                                  ...
                                  items: _operationCubit.project!.screens
                                      ...
                                      .toList(),
                                  onChanged: (value) {
                                    if (value != _operationCubit.project!.mainScreen) {
                                      _operationCubit.project!.mainScreen = value;
                      ...
                      IgnorePointer(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text('Target Devices', style: AppFontStyle.lato(14, fontWeight: FontWeight.w700)),
                            const SizedBox(
                              height: 10,
                            ),
                            Row(
                              children: [
                                for (final TargetPlatformType key in TargetPlatformType.values)
                                  Padding(
                                    padding: const EdgeInsets.only(right: 4),
                                    child: StatefulBuilder(builder: (context, setStateForTargetTile) {
                                      final value = project.settings.target[key] ?? true;
                                      return InkWell(
                                        onTap: () {
                                          undoWork.add(value, () {
                                            project.settings.target[key] = !value;
                                            setState(() {});
                                          }, (value) {
                                            project.settings.target[key] = value;
                                            setState(() {});
                                        ...
                                        child: AnimatedContainer(
                                          ...
                                          decoration: BoxDecoration(
                                              ...
                                              border: Border.all(
                                                  color: value ? ColorAssets.theme : ColorAssets.grey, width: 1.4)),
                              ...
                              child: Row(
                                children: [
                                  Radio(
                                      ...
                                      fillColor: WidgetStateProperty.all(ColorAssets.theme),
                      ...
                      if (setting.isPublic)
                        Column(
                          ...
                          children: [
                            Text(
                              'Paste this link',
                              style: AppFontStyle.lato(15, color: theme.text2Color, fontWeight: FontWeight.w500),
                            ),
                            const SizedBox(
                              height: 10,
                            ),
                            InkWell(
                              onTap: () {
                                Clipboard.setData(ClipboardData(text: '${appLink}run/${project.id}'));
                                    ...
                                    Expanded(
                                      child: Text(
                                        ...
                                        style: AppFontStyle.lato(15,
                                            color: theme.text2Color.withOpacity(0.6), fontWeight: FontWeight.w500),
                                      ),
                                    ),
                                  ],
                                ),
                        ...
                        BlocConsumer(
                          ...
                          builder: (context, state) {
                            return OverlayLoadingComponent(
                              ...
                              radius: 6,
                              size: 25,
                              child: StatefulBuilder(builder: (context, setStateForCollaborators) {
                                return Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Row(
                                    ...
                                    Row(
                                      children: [
                                        ...
                                        ListenableBuilder(
                                            ...
                                            builder: (context, _) {
                                              final bool enable = _collaboratorController.text.length >= 5;
                                              return InkWell(
                                                onTap: enable
                                                    ? () {
                                                        if (setting.collaborators?.firstWhereOrNull((element) =>
                                                                element.email == _collaboratorController.text) !=
                                                            null) {
                                                          showConfirmDialog(
                                                            title: 'Alert!',
                                                            subtitle: 'User is already a collaborator!',
                                                            context: context,
                                                            positive: 'Ok',
                                                          );
                                                        } else {
                                                          _settingsBloc.add(SettingsAddCollaboratorEvent(model));
                                                          model = FVBCollaborator(
                                                              email: '', permission: ProjectPermission.editor);
                                                        }
                                                      }
                                                    : null,
                                                borderRadius: BorderRadius.circular(10),
                                                child: CircleAvatar(
                                                  backgroundColor: enable ? ColorAssets.theme : ColorAssets.grey,
                                                  radius: 10,
                                                  child: const Icon(
                                                    Icons.done,
                                                    color: Colors.white,
                                    ...
                                    ListView.builder(
                                      ...
                                      itemBuilder: (_, index) => CollaboratorTile(
                                          model: setting.collaborators![index],
                                          onDelete: () {
                                            setting.collaborators?.remove(setting.collaborators![index]);
                                            _settingsBloc.add(SettingsUpdateCollaboratorsEvent());
                                          }),
                                      itemCount: setting.collaborators?.length ?? 0,
                                    ),
                                  ],
                                );
                              }),
                      ...
                      if (AppConfig.isAdmin && collection.project!.screens.isNotEmpty) ...[
                        15.hBox,
                        Container(
                    ...
                    children: [
                      if (!undoWork.isEmpty) ...[
                        FilledButtonWidget(
                          ...
                          onTap: () {
                            undoWork.clear();
                            context.read<OperationCubit>().updateProjectSettings();
  ...
  _buildPathSelection(String title, String? path, ValueChanged<String> onChanged) {
    String? value = path;
          ...
          Row(
            ...
            children: [
              ...
              FilledButtonWidget(
                ...
                onTap: () {
                  FilePicker.platform.getDirectoryPath(initialDirectory: value).then((_value) {
                    if (_value != null) {
...
class CollaboratorTile extends StatelessWidget {
  ...
  const CollaboratorTile({Key? key, required this.model, required this.onDelete}) : super(key: key);
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            Text(
              model.email,
              style: AppFontStyle.lato(14, color: theme.text2Color, fontWeight: FontWeight.normal),
            ),
            const SizedBox(
              width: 10,
            ),
            ...
            20.wBox,
            RoundedAppIconButton(icon: Icons.delete, onPressed: onDelete, color: ColorAssets.red)
          ],
        ),
      ),
    );
...
class PermissionTypeDropDown extends StatelessWidget {
  ...
  const PermissionTypeDropDown({Key? key, required this.model, required this.onChanged}) : super(key: key);
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<ProjectPermission>(
          ...
          items: const [
            ...
            DropdownMenuItem(child: Text('Editor'), value: ProjectPermission.editor),
          ],
          style: AppFontStyle.lato(14, color: theme.text1Color),
          onChanged: (value) {
            if (value != null) {
...
class SelectionGroupWidget<T> extends StatelessWidget {
  ...
  const SelectionGroupWidget(
      {Key? key, required this.selection, required this.data, required this.onChange, required this.title})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        Text(
          title,
          style: AppFontStyle.lato(14, color: theme.titleColor, fontWeight: FontWeight.w700),
        ),
        const Spacer(),
        SizedBox(
          width: 200,

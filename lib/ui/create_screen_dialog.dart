...
import 'common/ai_generation_section.dart';
    ...
    return BlocListener(
      ...
      child: Form(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(color: theme.background1, borderRadius: BorderRadius.circular(10)),
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: AppTextField(
                          ...
                          validator: (value) {
                            ...
                            if (_operationCubit.project?.screens.firstWhereOrNull((element) => element.name == value) != null) {
                      ...
                      AIGenerationSection(selectedComponent: generatedComponent),
                      const SizedBox(
                        height: 15,
                      ),
                          ...
                          child: Row(
                            children: [
                              ...
                              BlocBuilder(
                                bloc: _operationCubit,
                                buildWhen: (_, state) =>
                                    state is ComponentOperationLoadingFigmaScreensState ||
                                    state is ComponentOperationErrorState ||
                                    state is ComponentOperationInitial,
                                builder: (context, state) {
                                  if (state is ComponentOperationLoadingFigmaScreensState) {
                                    return const Align(
                                    ...
                                    onPressed: () {
                                      if (_figmaLink.text.isNotEmpty) {
                                        _operationCubit.addScreensFromFigma(
                                            _userSession.settingModel!.figmaAccessToken!, _figmaLink.text);
                                      }
                                    },
                            ...
                            child: Row(
                              ...
                              children: [
                                Text(
                                  ...
                                  style: AppFontStyle.lato(14, color: theme.text3Color, fontWeight: FontWeight.normal),
                                ),
                                10.wBox,
                                Icon(
  ...
  void _onCreate() async {
    if (_formkey.currentState!.validate()) {
      ...
      if(generatedComponent.value!=null){
        ...
        screen.rootComponent=generatedComponent.value;
      }
      else if (selectedTemplate == null || _figmaLink.text.isNotEmpty) {
        screen = Screen.otherScreen(name, _operationCubit.project!, type: type);
      } else {
        ...
        for (final custom in selectedTemplate!.customComponents) {
          ...
          comp.objects.forEach((element) {
            element.processor.parentProcessor = _operationCubit.project!.processor;
          });
        ...
        await Future.wait(selectedTemplate!.images.map((e) => _operationCubit.uploadImage(e)).toList());
        await _operationCubit.addVariables(selectedTemplate!.variables.map((e) => e.clone()).toList(growable: false));
      }
      _operationCubit
          .addScreen(
        screen,
        ...
        _componentSelectionCubit
            .init(ComponentSelectionModel.unique(screen.rootComponent!, screen.rootComponent!, screen: screen));
...
class TemplateSelectionWidget extends StatefulWidget {
  ...
  const TemplateSelectionWidget({Key? key, this.selection = true, this.onChange}) : super(key: key);
...
class _TemplateSelectionWidgetState extends State<TemplateSelectionWidget> {
  ...
  @override
  void initState() {
    componentOperationCubit.loadTemplates(widget.selection ? null : _userDetails.userId);
                        ...
                        child: Column(
                          ...
                          children: [
                            if (componentOperationCubit.templateList == null)
                              ...
                              Expanded(
                                flex: widget.selection ? 1 : 0,
                                child: (componentOperationCubit.templateList?.isEmpty ?? true)
                                    ? const Center(
                                  ...
                                  children: [
                                    if (widget.selection)
                                      InkWell(
                                        ...
                                        child: DottedBorder(
                                          ...
                                          strokeWidth: 2,
                                          color: selected == null ? ColorAssets.theme : ColorAssets.grey,
                                          child: Container(
                                            width: 100,
                                            height: 100,
                                            alignment: Alignment.center,
                                    ...
                                    ...componentOperationCubit.templateList!.asMap().entries.map((e) {
                                      final index = e.key;
                                      final template = componentOperationCubit.templateList![index];
                                      final screenConfig =
                                          defaultScreenConfigs.firstWhereOrNull((e) => e.name == template.device) ??
                                              screenConfigCubit.screenConfigs.first;
                                      final double width = 140;
                                      return Container(
                                        width: width,
                                        height: width * screenConfig.height / screenConfig.width,
                                        child: InkWell(
                                          onTap: () {
                                            selected = template;
                                            widget.onChange?.call(selected);
                                              ...
                                              Positioned.fill(
                                                child: Container(
                                                  decoration: BoxDecoration(
                                                    ...
                                                    border: Border.all(
                                                        width: 2,
                                                        color: selected == template ? ColorAssets.theme : ColorAssets.grey,
                                                        strokeAlign: BorderSide.strokeAlignOutside),
                                                  ),
                                                  child: EmulationView(
                                                    widget: template.screen.build(context) ?? const Offstage(),
                                              ...
                                              if (!widget.selection)
                                                Positioned(
                                                  ...
                                                  child: DeleteIconButton(
                                                    onPressed: () {
                                                      componentOperationCubit.deleteTemplate(template);
                                                    },
                                                  ),
                                                )
                                            ],

...
abstract class FireBridge {
  ...
  static Future<void> saveCustomComponent(FlutterProject project, CustomComponent customComponent, {String? newName}) async {
    if (newName != null && newName != customComponent.name) {
      await FirePath.customComponentsReferenceUpdated(project.userId,project.name,newName).set(customComponent.toJson());
      ...
      await FirePath.customComponentsReferenceUpdated(project.userId,project.name,customComponent.name).update({
        'code': CodeOperations.trim(customComponent.root?.code(clean: false)),
        'name': customComponent.name,
        'action_code': customComponent.actionCode,
        'variables': customComponent.
        variables.values
  ...
  static Future<void> addCustomComponent(
      int userId, FlutterProject flutterProject, CustomComponent customComponent) async {
    final String type;
    ...
    await FirePath.customComponentsReferenceUpdated(flutterProject.userId,flutterProject.name,customComponent.name).set({
      'name': customComponent.name,
      'action_code': customComponent.actionCode,
      'code': CodeOperations.trim(customComponent.root?.code(clean: false)),
  ...
  static Future<void> deleteCustomComponent(
      int userId, FlutterProject project, CustomComponent component) async {
  ...
  static Future<void> deleteProject(int userId, FlutterProject project, final List<FlutterProject> projects) async {
    ...
    if (Platform.isWindows) {
      await FirebaseFirestore.instance.collection('us$userId').doc(Strings.kFlutterProjectInfo).update(
  ...
  static Future<void> moveDocument(List<DocData> fromData, List<DocData> toData,
    ...
    late DocumentReference reference =
        FirebaseFirestore.instance.collection(fromData.first.collectId).doc(fromData.first.docId);
    ...
    late DocumentReference destReference =
        FirebaseFirestore.instance.collection(toData.first.collectId).doc(toData.first.docId);
  ...
  static Future<void> moveCollection(List<DocData> fromData, List<DocData> toData,
      {bool deleteOld = true, Map<String, dynamic>? additionalData,QuerySnapshot? snapshot,String? appendTargetDocKey}) async {
    final List<QueryDocumentSnapshot>? data;
    late CollectionReference reference = FirebaseFirestore.instance.collection(fromData.first.collectId);
    if(snapshot!=null) {
      for (final doc in fromData.sublist(1)) {
    ...
    late CollectionReference destReference = FirebaseFirestore.instance.collection(toData.first.collectId);
    for (final doc in toData.sublist(1)) {
      destReference = destReference.doc(doc.docId).collection(doc.collectId);
    }
    for (final QueryDocumentSnapshot doc in data ?? []) {
      await destReference.doc(doc.id+(appendTargetDocKey??'')).set((doc.data() as Map<String, dynamic>)..addAll(additionalData ?? {}));
      if (deleteOld) {
  ...
  static Future<List<FavouriteModel>> loadFavourites(final int userId) async {
    ...
    snapshot = await FirebaseFirestore.instance.collection('us$userId|${Strings.kFavourites}').get();

    final List<FavouriteModel> favouriteModels = [];
    for (final document in snapshot.docs) {
      final json = document.data();
      favouriteModels.add(FavouriteModel(
          Component.fromCode(json['code'], ComponentOperationCubit.currentProject!)!
  ...
  static Future<List<CustomComponent>> loadAllCustomComponents(final int userId, String name) async {
    final snapshot =
        await FirePath.customComponentsReferenceByProjectNameUpdated(userId).where('project', isEqualTo: name).get();
    return snapshot.docs.map((e) => CustomComponent.fromJson(e.data()! as Map<String,dynamic>)).toList();
  }
  ...
  static Future<void> removeFromFavourites(int userId, Component component) async {
  ...
  static Future<AuthResponse> registerUser(String userName, String password) async {
    ...
    final doc = await FirebaseFirestore.instance.collection('userInfo').doc('count').get();
    if (doc.data() == null || !doc.exists) {
      return AuthResponse.right('No user data exist');
    }

    final newUserId = doc.data()!['count'] + 1;
    await FirebaseFirestore.instance.collection('userInfo').doc('count').update({'count': newUserId});
    await FirebaseFirestore.instance
        .collection('users')
        .add({'username': userName, 'password': password, 'userId': newUserId, 'uid': user.user!.uid});
    await FirebaseFirestore.instance.collection('us$newUserId').doc(Strings.kFlutterProjectInfo).set({'projects': []});
    // await FirebaseFirestore.instance
  ...
  static Future<List<FlutterProject>> loadAllFlutterProjects(
    ...
    final snapshot = await FirebaseFirestore.instance.collection('us$userId').doc(Strings.kFlutterProjectInfo).get();
  ...
  static Future<void> saveFlutterProject(int userId, FlutterProject project) async {
    ...
    final response = await FirebaseFirestore.instance
        ...
        .collection(project.name)
        .add(projectInfo);
    final jsonList = project.uiScreens.map((e) => e.toJson()).toList(growable: false);
    ...
    if (Platform.isWindows) {
      final oldResponse =
          await FirebaseFirestore.instance.collection('us$userId').doc(Strings.kFlutterProjectInfo).get();
      await FirebaseFirestore.instance
          .collection('us$userId')
  ...
  static Future<Optional<FlutterProject, ProjectLoadErrorModel>> loadFlutterProject(int userId, String name,
      {bool ifPublic = false}) async {
    try {
      final snapshot =
          await FirebaseFirestore.instance.collection('us$userId').doc(Strings.kFlutterProject).collection(name).get();
      ...
      final FlutterProject flutterProject = FlutterProject(projectInfo['project_name'], userId, projectInfoDoc.id,
      ...
      late List<QueryDocumentSnapshot> customDocuments;
      await Migrator.migrateIfNeeded(() async {
        customDocuments = (await FirePath.customComponentsReferenceByProjectNameUpdated(userId)
            .where('project', isEqualTo: name)
            .get())
            .docs;

        return customDocuments.isEmpty;
      }, () async {
        ...
        await moveCollection([DocData('us$userId', ''), DocData('Custom|${flutterProject.docId}', Strings.kFlutterProject)],
            [DocData('us$userId', ''), DocData('Custom', Strings.kFlutterProject)],
            additionalData: {'project': flutterProject.name},snapshot: customDocs,deleteOld: false,appendTargetDocKey: '|${flutterProject.name}');
      });
      ...
      for (final doc in customDocuments) {
        ...
        flutterProject.customComponents.add(CustomComponent.fromJson(componentBody));
      }
      ...
      for (int i = 0; i < customDocuments.length; i++) {
        final Map<String, dynamic> componentBody = customDocuments[i].data()! as Map<String, dynamic>;
        flutterProject.customComponents[i].root =
            componentBody['code'] != null ? Component.fromCode(componentBody['code']!, flutterProject) : null;
      }
      for (int i = 0; i < customDocuments.length; i++) {
        flutterProject.customComponents[i].root?.forEach((p0) {
          if (p0 is CustomComponent) {
            p0.root =
                flutterProject.customComponents.firstWhere((element) => element.name == p0.name).root?.clone(null);
          }
        });
      }
  ...
  static Future<List<ImageData>?> loadAllImages(int userId) async {
    final QuerySnapshot<Map<String, dynamic>> image;
    image = await FirebaseFirestore.instance.collection('us$userId|${Strings.kImages}').get();
    if (image.docs.isNotEmpty) {
  ...
  static Future<void> uploadImage(int userId, String projectName, ImageData imageData) async {
    await FirebaseFirestore.instance
        .collection('us$userId|${Strings.kImages}')
        .add({'img_name': imageData.imageName, 'project_name': projectName, 'bytes': base64Encode(imageData.bytes!)});
  }
  ...
  static Future<void> addModel(final int userId, final FlutterProject project, final LocalModel localModel) async {
    await FirebaseFirestore.instance
        .collection('us$userId')
        .doc(Strings.kFlutterProject)
        .collection(project.name)
  ...
  static Future<void> updateModel(final int userId, final FlutterProject project, final LocalModel localModel) async {
    await FirebaseFirestore.instance
        .collection('us$userId')
        .doc(Strings.kFlutterProject)
        .collection(project.name)
        .doc(project.currentScreen.name)
        .update({'models': project.currentScreen.models.map((e) => e.toJson()).toList(growable: false)});
    debugPrint('=== FIRE-BRIDGE == update variable == ');
  }

  static Future<void> addVariable(
      final int userId, final FlutterProject project, final VariableModel variableModel) async {
    if (Platform.isWindows) {
  ...
  static Future<void> addVariableForScreen(
      final int userId, final FlutterProject project, final VariableModel variableModel) async {
    if (Platform.isWindows) {
      final variablesRef = FirebaseFirestore.instance
          .collection('us$userId')
  ...
  static Future<void> updateVariable(final int userId, final FlutterProject project) async {
    await FirebaseFirestore.instance
        .collection('us$userId')
        .doc(Strings.kFlutterProject)
        .collection(project.name)
  ...
  static Future<void> updateUIScreenVariable(final int userId, final FlutterProject project) async {
    await FirebaseFirestore.instance
        .collection('us$userId')
        .doc(Strings.kFlutterProject)
        .collection(project.name)
  ...
  static Future<void> updateCustomComponentActionCode(
      int userId, final FlutterProject project, final CustomComponent component) async {
    await FirePath.customComponentsReferenceUpdated(project.userId,project.name,component.name).update({
      'action_code': component.actionCode,
    });
  }
  static Future<void> updateDeviceSelection(final int userId, final FlutterProject project, final String device) async {
    await FirebaseFirestore.instance
        .collection('us$userId')
        .doc(Strings.kFlutterProject)
        .collection(project.name)
  ...
  static Future<void> updateSettings(final int userId, final FlutterProject project) async {
    await FirebaseFirestore.instance
        .collection('us$userId')
        .doc(Strings.kFlutterProject)
        .collection(project.name)
  ...
  static Future<void> updateCurrentScreen(int userId, final FlutterProject project) async {
    await FirebaseFirestore.instance
        .collection('us$userId')
        .doc(Strings.kFlutterProject)
        .collection(project.name)
  ...
  static Future<void> updateMainScreen(int userId, final FlutterProject project) async {
    await FirebaseFirestore.instance
        .collection('us$userId')
        .doc(Strings.kFlutterProject)
        .collection(project.name)
  ...
  static Future<void> updateProjectValue(final FlutterProject project, String key, dynamic value) async {
    await FirebaseFirestore.instance
  ...
  static Future<void> updateActionCode(int userId, final FlutterProject project) async {
    await FirebaseFirestore.instance
        .collection('us$userId')
        .doc(Strings.kFlutterProject)
        .collection(project.name)
  ...
  static Future<void> updateScreenActionCode(int userId, final FlutterProject project) async {
    await FirebaseFirestore.instance
        .collection('us$userId')
        .doc(Strings.kFlutterProject)
        .collection(project.name)
  ...
  static Future<void> addUIScreen(final int userId, final FlutterProject project, final UIScreen uiScreen) async {
  ...
  static Future<void> updateScreenRootComponent(
      final int userId, final String projectName, final UIScreen uiScreen, final Component component) async {
    await FirebaseFirestore.instance
        .collection('us$userId')
        .doc(Strings.kFlutterProject)
        .collection(projectName)
...
abstract class FirePath {
  ...
  static CollectionReference customComponentsReferenceByProjectNameUpdated(int userId) {
    return FirebaseFirestore.instance.collection('us$userId').doc(Strings.kFlutterProject).collection('Custom');
  }
  static DocumentReference customComponentsReferenceUpdated(int userId,String project,String name) {
    return FirebaseFirestore.instance.collection('us$userId').doc(Strings.kFlutterProject).collection('Custom').doc(name+'|'+project);
  }
}

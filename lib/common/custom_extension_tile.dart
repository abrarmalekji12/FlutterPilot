...
final Map<String, List<_ExpansionTileState>> _map = {};

class ExpansionTileProvider extends ChangeNotifier {
  toggle() {
    notifyListeners();
  }
}
...
class CustomExpansionTile extends StatefulWidget {
  ...
  static ExpansionTileProvider of(BuildContext context) {
    return context.read<ExpansionTileProvider>();
  }
}
...
class _ExpansionTileState extends State<CustomExpansionTile>
  ...
  final ExpansionTileProvider _provider = ExpansionTileProvider();

  @override
  void initState() {
    super.initState();
    if (widget.groupName != null) {
      if (_map.containsKey(widget.groupName)) {
        _map[widget.groupName]!.add(this);
      } else {
        _map[widget.groupName!] = [this];
      }
    }
    ...
    _provider.removeListener(_onChangeRequest);
    _provider.addListener(_onChangeRequest);
  }
  ...
  void dispose() {
    _controller.dispose();
    if (widget.groupName != null) {
      _map[widget.groupName!]!.remove(this);
    }
    super.dispose();
  }
        ...
        children: <Widget>[
          ListTileTheme.merge(
            ...
            child: InkWell(
              onTap: () {
                if (widget.groupName != null) {
                  for (final _notifier in _map[widget.groupName]!) {
                    if (_notifier != this && _notifier._isExpanded) {
                      _notifier._handleTap();
                    }
                  }
                }
                _handleTap();
              ...
              child: Padding(
                ...
                child: Row(
                  children: [
                    ...
                    widget.subtitle ?? const Offstage(),
                    widget.trailing ??

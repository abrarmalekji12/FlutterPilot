...
class WordBoundary extends TextBoundary {
  ...
  static int _codePointFromSurrogates(int highSurrogate, int lowSurrogate) {
    assert(
      AppTextPainter.isHighSurrogate(highSurrogate),
      'U+${highSurrogate.toRadixString(16).toUpperCase().padLeft(4, "0")}) is not a high surrogate.',
    );
    assert(
      AppTextPainter.isLowSurrogate(lowSurrogate),
...
class _TextLayout {
  ...
  double get minIntrinsicLineExtent =>
      _applyFloatingPointHack(_paragraph.minIntrinsicWidth);

  /// The width at which increasing the width of the text no longer decreases the height.
  ///
  /// Includes trailing spaces if any.
  double get maxIntrinsicLineExtent =>
      _applyFloatingPointHack(_paragraph.maxIntrinsicWidth);
...
///
/// To use a [AppTextPainter], follow these steps:
///
/// 1. Create a [TextSpan] tree and pass it to the [AppTextPainter]
///    constructor.
///
...
///    native resources. For [AppTextPainter] objects that are used repeatedly and
///    stored on a [State] or [RenderObject], call [dispose] from
///    [State.dispose] or [RenderObject.dispose] or similar. For [AppTextPainter]
...
class AppTextPainter {
  ...
  /// Computes the width of a configured [AppTextPainter].
  ///
  ...
  /// returns its [AppTextPainter.width] making sure to dispose the underlying
  /// resources. Doing this operation is expensive and should be avoided
  /// whenever it is possible to preserve the [AppTextPainter] to paint the
  ...
  /// Computes the max intrinsic width of a configured [AppTextPainter].
  ///
  /// This is a convenience method that creates a text painter with the supplied
  /// parameters, lays it out with the supplied [minWidth] and [maxWidth], and
  /// returns its [AppTextPainter.maxIntrinsicWidth] making sure to dispose the
  ...
  /// representation of the contents of this [AppTextPainter], use [plainText].
  ...
  /// that you pass to the [AppTextPainter] constructor or to the [text] property.
  ...
  ///
  /// Valid only after [layout] has been called.
  Rect getRectForCaret(TextPosition position, Rect caretPrototype) {
    ...
    switch (caretMetrics) {
      case _EmptyLineCaretMetrics(:final double lineVerticalOffset):
        ...
        return Rect.fromLTWH(dx, lineVerticalOffset, 0, 0);
    ...
    return Rect.fromLTWH(adjustedDx, rawOffset.dy + layoutCache.paintOffset.dy,
        caretMetrics.width, caretMetrics.fullHeight);
  }

  ...
  static List<dynamic> process(final CodeProcessor processor,
      ...
      if (arguments[i].type == FVBArgumentType.placed ||
        ...
        final output = argumentData.length > i
            ...
            : (arguments[i].type == FVBArgumentType.optionalPlaced
                ...
                : throw Exception(
                    'Missing argument ${arguments[i].name} in function $name'));
    ...
    for (int i = 0; i < processedArguments.length; i++) {
      if (processedArguments[i] is FVBTest) {
        processedArguments[i] =
            (processedArguments[i] as FVBTest).testValue(processor);
      }
    }
      ...
      if (type == FVBArgumentType.placed) {
        if (argumentList[i].startsWith('this.')) {
          ...
          final value = DataTypeProcessor.getFVBValueFromCode(
              argumentList[i], CodeProcessor.classes, CodeProcessor.enums);
        ...
        if (split.length == 2) {
          final value = DataTypeProcessor.getFVBValueFromCode(
              split[0], CodeProcessor.classes, CodeProcessor.enums);
          arguments.add(FVBArgument(

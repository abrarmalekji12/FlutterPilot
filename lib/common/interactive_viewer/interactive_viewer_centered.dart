...
/// See also:
///
///   * [OldCustomInteractiveViewer.builder], whose builder is of this type.
...
/// {@end-tool}
@immutable
class OldCustomInteractiveViewer extends StatefulWidget {
  /// Create an InteractiveViewer.
  ///
  /// The [child] parameter must not be null.
  OldCustomInteractiveViewer({
    Key? key,
    ...
    required Widget this.child,
        ...
        builder = null,
        super(key: key);
  final ValueNotifier<bool>? dragEnable;
  ...
  OldCustomInteractiveViewer.builder({
    Key? key,
    this.clipBehavior = Clip.hardEdge,
    this.dragEnable,
    this.alignPanAxis = false,
  ...
  ///
  /// Passed with the [OldCustomInteractiveViewer.builder] constructor. Otherwise, the
  ...
  ///
  /// If the [OldCustomInteractiveViewer.builder] constructor is used, then this will be
  ...
  ///
  /// At the time this is called, the [OldTransformationController] will have
  ...
  ///
  /// At the time this is called, the [OldTransformationController] will not have
  ...
  /// [OldTransformationController.toScene] for how to convert these coordinates to
  ...
  /// A [OldTransformationController] for the transformation performed on the
  /// child.
  ///
  ...
  final OldTransformationController? transformationController;
  ...
  static Vector3 getNearestPointInside(Vector3 point, Quad quad) {
    ...
    final List<Vector3> closestPoints = <Vector3>[
      OldCustomInteractiveViewer.getNearestPointOnLine(
  ...
  @override
  State<OldCustomInteractiveViewer> createState() =>
      _OldCustomInteractiveViewerState();
}

class _OldCustomInteractiveViewerState extends State<OldCustomInteractiveViewer>
    with TickerProviderStateMixin {
  OldTransformationController? _transformationController;
  ...
  Matrix4 _matrixScale(Matrix4 matrix, double scale) {
    ...
    widget.scaleNotifier?.value =
        ((widget.scaleNotifier?.value ?? 1) * clampedScale);
  ...
  bool _gestureIsSupported(_GestureType? gestureType) {
    switch (gestureType) {
      ...
      case null:
        return (widget.dragEnable?.value) ?? true;
    }
  }
  ...
  void _onScaleUpdate(ScaleUpdateDetails details) {
    ...
    switch (_gestureType!) {
      case _GestureType.scale:
        ...
        // the scale.
        print('FOCAL POINT ${_referenceFocalPoint}');
  ...
  void _receivedPointerSignal(PointerSignalEvent event) {
    ...
    if (event is PointerScrollEvent) {
      ...
      final zoomEnable = (HardwareKeyboard.instance.physicalKeysPressed
              .contains(PhysicalKeyboardKey.controlLeft)) ||
          (HardwareKeyboard.instance.physicalKeysPressed
              .contains(PhysicalKeyboardKey.altLeft));
      if (!zoomEnable || !_gestureIsSupported(_GestureType.scale)) {
        final horizontal = HardwareKeyboard.instance.physicalKeysPressed
        ...
        _transformationController!.value = _transformationController!.value
            .clone()
          ..translate(horizontal ? -event.scrollDelta.dy * 0.5 : 0.0,
              !horizontal ? -event.scrollDelta.dy * 0.5 : 0.0);
  ...
  void initState() {
    ...
    _transformationController =
        widget.transformationController ?? OldTransformationController();
  ...
  @override
  void didUpdateWidget(OldCustomInteractiveViewer oldWidget) {
    ...
    if (oldWidget.transformationController == null) {
      ...
      if (widget.transformationController == null) {
        _transformationController!
            .removeListener(_onTransformationControllerChange);
        _transformationController = OldTransformationController();
        _transformationController!
            .addListener(_onTransformationControllerChange);
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(builder: (context, constraints) {
      return Listener(
        ...
        onPointerPanZoomStart: (evnet) {
          print('Zoom Start: $evnet');
        },
        // onPointerPanZoomUpdate: (event) {
        //   if (event.scale == 1) {
        //     _transformationController!.value = _transformationController!.value.clone()
        //       ..translate(event.panDelta.dx * 0.5, event.panDelta.dy * 0.5);
        //
        //     print(
        //         'SEE Update: ${event.scale} ${event.pan} ${event.panDelta} ${event.size} ${event.rotation} ${event.distance} ${event.buttons}');
        //   }
        // },
        onPointerMove: (event) {
          print('MOVE ${event}');
          if (event.down &&
              HardwareKeyboard.instance.physicalKeysPressed
                  .contains(PhysicalKeyboardKey.space)) {
            _transformationController!.value = _transformationController!.value
                .clone()
              ..translate(
                  event.delta.dx * scaleChange, event.delta.dy * scaleChange);
          }
        },
...
/// See also:
///
///  * [OldCustomInteractiveViewer.transformationController] for detailed documentation
///    on how to use TransformationController with [InteractiveViewer].
class OldTransformationController extends ValueNotifier<Matrix4> {
  /// Create an instance of [OldTransformationController].
  ///
  /// The [value] defaults to the identity matrix, which corresponds to no
  /// transformation.
  OldTransformationController([Matrix4? value])
...
Quad _getAxisAlignedBoundingBoxWithRotation(Rect rect, double rotation) {
  ...
  return OldCustomInteractiveViewer.getAxisAlignedBoundingBox(
      boundariesRotated);
}
...
Offset _exceedsBy(Quad boundary, Quad viewport) {
  ...
  for (final Vector3 point in viewportPoints) {
    final Vector3 pointInside =
        OldCustomInteractiveViewer.getNearestPointInside(point, boundary);

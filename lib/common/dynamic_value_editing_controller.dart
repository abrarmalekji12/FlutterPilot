...
class DynamicValueEditingController extends TextEditingController {
  ...
  TextSpan buildTextSpan({required BuildContext context,
    ...
    for (int i = 0; i < indexList.length; i += 2) {
      spans.add(TextSpan(text: textValue.substring(start, indexList[i]),style: style));
      final bold=textValue.substring(indexList[i], indexList[i + 1] + 2);
      spans.add(TextSpan(
          text: bold,
          style: AppFontStyle.roboto(13, color: bold.contains(' ')?Colors.red:AppColors.theme,fontWeight: FontWeight.bold)));
      start=indexList[i + 1] + 2;
    }
    spans.add(TextSpan(text: textValue.substring(start, textValue.length),style: style));

...
import '../ui/component_tree/component_tree.dart';
import '../ui/home/home_page.dart';

/// Signature for determining whether the given data will be accepted by a [CustomDragTarget].
///
/// Used by [CustomDragTarget.onWillAccept].
typedef DragTargetWillAccept<T> = bool Function(T? data, Offset position);

/// Signature for causing a [CustomDragTarget] to accept the given data.
///
/// Used by [CustomDragTarget.onAccept].
typedef DragTargetAccept<T> = void Function(T data, Offset position);

/// Signature for determining information about the acceptance by a [CustomDragTarget].
///
/// Used by [CustomDragTarget.onAcceptWithDetails].
...
typedef DragTargetBuilder<T> = Widget Function(BuildContext context,
    List<T?> candidateData, List<dynamic> rejectedData, Offset? position);

/// Signature for when a [CustomDraggable] is dragged across the screen.
///
/// Used by [CustomDraggable.onDragUpdate].
...
///
/// Used by [CustomDraggable.onDraggableCanceled].
...
///
/// Used by [CustomDraggable.onDragEnd].
typedef DragEndCallback = void Function(DraggableDetails details);

/// Signature for when a [CustomDraggable] leaves a [CustomDragTarget].
///
/// Used by [CustomDragTarget.onLeave].
typedef DragTargetLeave<T> = void Function(T? data);

/// Signature for when a [CustomDraggable] moves within a [CustomDragTarget].
///
/// Used by [CustomDragTarget.onMove].
...
///  * [CustomDraggable.dragAnchorStrategy], for which this is a built-in value.
...
/// See also:
///
///  * [CustomDraggable], similar to the [CustomLongPressDraggable] widget but happens immediately.
///  * [CustomDragTarget], a widget that receives data when a [CustomDraggable] widget is dropped.
class CustomLongPressDraggable<T extends Object> extends CustomDraggable<T> {
...
/// when the draggable event occurred, and whether its [CustomDragTarget] accepted it.
///
...
class DraggableDetails {
  ...
  /// Determines whether the [CustomDragTarget] accepted this draggable.
...
/// Represents the details when a pointer event occurred on the [CustomDragTarget].
class DragTargetDetails<T> {
  /// Creates details for a [CustomDragTarget] callback.
  ///
  /// The [offset] must not be null.
  DragTargetDetails({required this.data, required this.offset});

  /// The data that was dropped onto this [CustomDragTarget].
  final T data;
...
///
///  * [CustomDraggable], similar to the [CustomLongPressDraggable] widget but happens immediately.
///  * [DragTarget], a widget that receives data when a [CustomDraggable] widget is dropped.
...
class _CustomDraggableState<T extends Object>
  ...
  _DragAvatar<T>? _startDrag(Offset position) {
    final bloc = context.read<ComponentDragBloc>();
    if (widget.maxSimultaneousDrags != null &&
        _activeCount >= widget.maxSimultaneousDrags!) {
      return null;
    }
    ...
    final _DragAvatar<T> avatar = _DragAvatar<T>(
      ...
      onDragUpdate: (DragUpdateDetails details) {
        if (mounted && widget.onDragUpdate != null) {
          ...
          if (details.delta.dy < 1) {
            _scrollTimer?.cancel();
            if (details.globalPosition.dx < componentTreeWidth) {
              if (details.globalPosition.dy >
                  MediaQuery.of(context).size.height - 150) {
                componentTreeScrollController.animateTo(
                    componentTreeScrollController.offset + 100,
              ...
              } else if (details.globalPosition.dy < 150) {
                componentTreeScrollController.animateTo(
                    componentTreeScrollController.offset - 100,
                    duration: const Duration(milliseconds: 400),
                    curve: Curves.easeOut);
      ...
      onDragEnd: (Velocity velocity, Offset offset, bool wasAccepted) {
        ...
        bloc.add(ComponentInitialEvent());
    ...
    bloc.add(ComponentDraggingEvent(widget.area));
...
class _CustomDragTargetState<T extends Object>
  ...
  bool didEnter(_DragAvatar<Object> avatar) {
    ...
    if (widget.onWillAccept == null ||
        widget.onWillAccept!(avatar.data as T?, avatar._position)) {
      position = avatar._position;
      setState(() {
  ...
  void didDrop(_DragAvatar<Object> avatar) {
    ...
    widget.onAccept?.call(avatar.data! as T, position!);
  ...
  Widget build(BuildContext context) {
    return MetaData(
      ...
      child: widget.builder(context, _mapAvatarsToData<T>(_candidateAvatars),
          _mapAvatarsToData<Object>(_rejectedAvatars), position),
    );
  }
}
